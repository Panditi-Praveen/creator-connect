 ...
 *       {@link ProjectPrincipal} (userId + email + role) — the userId claim is
 *       the identity every project operation is scoped to.</li>
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private void authenticateIfValid(String token) {
        try {
            if (jwtService.isValid(token)) {
                ...
                setAuthentication(new ProjectPrincipal(userId, email, role));
            } else {
     ...
     */
    private void setAuthentication(ProjectPrincipal principal) {
