 ...
 *       {@link ProfilePrincipal} (userId + email + role) — the userId claim is
 *       the identity every profile operation is scoped to.</li>
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private void authenticateIfValid(String token) {
        try {
            if (jwtService.isValid(token)) {
                ...
                setAuthentication(new ProfilePrincipal(userId, email, role));
            }
     ...
     */
    private void setAuthentication(ProfilePrincipal principal) {
