...
/**
 * Centralized error handling for the whole Hiring Service.
 *
...
public class GlobalExceptionHandler {
    ...
    /**
     * Handles requests for an application that does not exist.
     *
    ...
    /**
     * Handles operations a user is not allowed to perform (wrong role, or a
     * freelancer touching someone else's application).
     *
     * @param ex      the thrown exception
     * @param request the originating HTTP request
    ...
    public ResponseEntity<ErrorResponse> handleApplicationAccessDenied(ApplicationAccessDeniedException ex,
                                                                       HttpServletRequest request) {
        return build(HttpStatus.FORBIDDEN, ex.getMessage(), request);
    }

    /**
     * Handles attempts to apply to a project the freelancer already applied
     * to.
     *
     * @param ex      the thrown exception
     * @param request the originating HTTP request
    ...
    /**
     * Handles illegal status transitions (deciding on or withdrawing an
     ...
     * express (e.g. a creator decision other than ACCEPTED/REJECTED).
     *
     * @param ex      the thrown exception
     * @param request the originating HTTP request
     ...
     * (e.g. a non-UUID {@code id} in {@code /applications/{id}}).
     *
     ...
     * index on {@code applications(project_id, freelancer_id)}).
     *
     * <p>Covers the race between the {@code existsByProjectIdAndFreelancerId}
     * check and the insert: if two identical applies arrive concurrently, the
     * database constraint wins and must still surface as {@code 409 CONFLICT}
     * rather than 500.
     *
     * @param request the originating HTTP request
