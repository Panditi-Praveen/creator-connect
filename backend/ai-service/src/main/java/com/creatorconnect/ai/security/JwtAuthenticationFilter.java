 ...
 *       {@link UsernamePasswordAuthenticationToken} whose principal is an
 *       {@link AiPrincipal} (userId + email + role).</li>
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private void authenticateIfValid(String token) {
        try {
            if (jwtService.isValid(token)) {
                ...
                setAuthentication(new AiPrincipal(userId, email, role));
            }
     ...
     */
    private void setAuthentication(AiPrincipal principal) {
